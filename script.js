// Get references to the buttons and the displayed image element
const button1 = document.getElementById('button1');
const button2 = document.getElementById('button2');
const button3 = document.getElementById('button3');
const displayedImage = document.getElementById('displayedImage');

// Set event listeners for each button
button1.addEventListener('click', () => {
  displayedImage.src = 'https://media.discordapp.net/attachments/1037900757585907772/1042018372793139360/starwatchernilla.png';
});

button2.addEventListener('click', () => {
  displayedImage.src = 'https://cdn.discordapp.com/attachments/854544686789165076/1125309497527779429/f45726b8e93bd5772146b8ebedea56e0.png';
});

button3.addEventListener('click', () => {
  displayedImage.src = 'https://cdn.discordapp.com/attachments/555738743953162240/1124914217456250963/176.jpg';
});

const opacitySlider = document.getElementById('opacitySlider');
const images = document.querySelectorAll('img');

opacitySlider.addEventListener('input', function() {
  images.forEach(function(image) {
    const opacityValue = opacitySlider.value / 100;
    image.style.opacity = opacityValue;
  });
});
